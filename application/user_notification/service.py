...
class Service:
    ...
    @utils.handle_service_error
    def post_message_team_general(self, team_id, message):
        conversation_list_url = 'https://slack.com/api/conversations.list'
        post_mess_url = 'https://slack.com/api/chat.postMessage'

        session = requests.Session()
        response = session.post(conversation_list_url, data={
            "token": Config.SLACK_PYCHATBOT_TOKEN,
            "team_id": team_id
        })
        channel_general = None
        for channel in json.loads(response._content.decode('utf-8'))["channels"]:
            if channel["is_general"]:
                channel_general = channel["id"]
                break

        stdout(channel_general)

        response = requests.post(post_mess_url, data={
            "token": Config.SLACK_PYCHATBOT_TOKEN,
    ...
    def get_all_project(self):
        return repository.get_all_project()
