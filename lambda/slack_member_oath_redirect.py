...
def sign_up_token(http, prefix, request_token_res):
    sign_up_token_response = http.request(
        "POST",
        f"{prefix}/project/token/member-monitor/sign-up",
...
def lambda_handler(event, context):
    ...
    if sign_up_token_res["status"] == "failed":
        ...
        get_all_user_info_res = get_all_user_info(
            http, request_token_res["access_token"], team_id
        )

        http.request(
            "POST",
            f'{get_parameters("PyChatbot-BE")}/user/list/sign-up',
            body=json.dumps(get_all_user_info_res),
            headers={"Content-Type": "application/json"},
            retries=False,
        )
